...
- Always run root `npm run typecheck` (and package checks `npm run check -w pi-tian-*` for modified packages) before pushing. The publish workflow runs all 13 checks plus `test:usage`, the pi-usage package tests, and the ask-user, commit, edit-safe, repo-model, repo-skills, subagents, compact-output, goal, image-cache, todo, token-speed, background-terminals, vscode-bridge, and web-search test suites before publishing.
