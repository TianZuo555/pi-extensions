...
import type { RemoteCompactionProtocolSetting } from "./model-api.ts";

export const COMPACT_SETTINGS_FILE = "pi-compact.json";
...
export interface CompactSettingsState {
...
export interface CompactSettingsRuntime {
  get(): Readonly<CompactSettingsState>;
  reload(signal?: AbortSignal): Promise<Readonly<CompactSettingsState>>;
  update(
    patch: Partial<CompactSettings>,
    signal?: AbortSignal,
  ): Promise<Readonly<CompactSettingsState>>;
  flush(): Promise<void>;
}
...
export function normalizeCompactSettings(value: unknown): CompactSettings | undefined {
  ...
  if (
    ...
    (typeof value.compactionModel !== "string" ||
      ...
      (value.compactionModel.length > 0 &&
        parseCompactionModelRef(value.compactionModel) === undefined))
  ) {
    return undefined;
  }
  ...
  return {
    ...
    compactionModel:
      typeof value.compactionModel === "string"
        ...
        : DEFAULT_CODEX_COMPACT_SETTINGS.compactionModel,
  };
}

export function compactSettingsPath(): string {
  return join(getAgentDir(), COMPACT_SETTINGS_FILE);
}
...
export async function loadCompactSettings(
  path = compactSettingsPath(),
  signal?: AbortSignal,
): Promise<CompactSettingsState> {
  aborted(signal);
  try {
    ...
    const settings = normalizeCompactSettings(document);
...
async function savePatch(
  ...
  const latest = await loadCompactSettings(path, signal);
  if (latest.kind === "invalid") {
    throw new Error("Cannot overwrite an invalid pi-compact.json; repair it and reload first");
  }
  ...
  try {
    ...
    aborted(signal);
    const current = await loadCompactSettings(path, signal);
    if (
      current.kind === "invalid" ||
      current.kind !== latest.kind ||
      JSON.stringify(current.document) !== JSON.stringify(latest.document)
    ) {
      throw new Error("pi-compact.json changed while saving; reopen settings and retry");
    }
...
export function createCompactSettingsRuntime(path = compactSettingsPath()): CompactSettingsRuntime {
  ...
  return {
    ...
    reload: (signal) =>
      enqueue(async () => {
        state = await loadCompactSettings(path, signal);
