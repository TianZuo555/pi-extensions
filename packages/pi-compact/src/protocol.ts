...
export class RemoteCompactionProtocolError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "RemoteCompactionProtocolError";
  }
}
...
export function validateCompactionItem(
  ...
  if (byteLength(value) > maxBytes) {
    throw new RemoteCompactionProtocolError("Remote compaction item exceeded the size limit");
  }
...
export async function collectCompactionSse(
  ...
  const dispatch = () => {
    ...
    try {
      ...
      throw new RemoteCompactionProtocolError("Remote compaction returned malformed SSE JSON");
    }
  ...
  try {
    while (true) {
      ...
      if (bytes > maxBytes) {
        throw new RemoteCompactionProtocolError("Remote compaction stream exceeded the size limit");
      }
...
export function validateCompactedResponse(
  ...
  if (!isObject(value) || !Array.isArray(value.output)) {
    throw new RemoteCompactionProtocolError(
      "Responses Compact returned an invalid response object",
    );
  }
  const maxBytes = options.maxBytes ?? MAX_COMPACT_JSON_BYTES;
  const maxItemBytes = options.maxItemBytes ?? MAX_COMPACTION_ITEM_BYTES;
  if (byteLength(value) > maxBytes) {
    throw new RemoteCompactionProtocolError("Responses Compact response exceeded the size limit");
  }
  if (value.output.length === 0) {
    throw new RemoteCompactionProtocolError("Responses Compact returned no output items");
  }
  const output = value.output.map((item) => {
    if (!isObject(item)) {
      throw new RemoteCompactionProtocolError(
        "Responses Compact returned a non-object output item",
      );
    }
...
export async function collectCompactResponse(
  ...
  if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
    const error = new RemoteCompactionProtocolError(
  ...
  if (!response.body) {
    throw new RemoteCompactionProtocolError("Responses Compact response did not contain a body");
  }
  ...
  try {
    ...
    throw new RemoteCompactionProtocolError("Responses Compact returned malformed JSON");
  }
...
export function rewriteCheckpointMarker(
  ...
  if (!isObject(payload) || !Array.isArray(payload.input)) {
    throw new RemoteCompactionProtocolError("Codex Responses payload is missing an input array");
  }
...
export function expandRemoteCompactionPayload(
  ...
  if (!isObject(payload) || !Array.isArray(payload.input)) {
    throw new RemoteCompactionProtocolError("Responses payload is missing an input array");
  }
