...
export const FIND_TOOL_DESCRIPTION =
  `Find files by whole-path match, backed by fd. pattern is matched against the whole repo-relative path: literal substring by default, regex when it contains regex syntax, and whitespace-separated words must all appear in any order. Respects .gitignore. Default limit ${DEFAULT_FIND_LIMIT}.`;

export const FIND_PROMPT_SNIPPET =
  "Find files by whole-path substring or regex match (fd-backed)";
...
export const FIND_PARAMETER_DESCRIPTIONS = {
  pattern:
    "Path query, matched against the whole repo-relative path. Literal substring by default; treated as a regex when it contains regex syntax (a regex that does not compile is retried as a literal). Whitespace-separated words must all match, in any order. Pass an empty string to list files matching path alone.",
  path: PATH_DSL_DESCRIPTION,
  exclude: EXCLUDE_DSL_DESCRIPTION,
  limit:
