...
import {
  buildRgArgs, clipLine, createSearchRuntime, finalizeGrep, runSearch, SearchRuntime,
  type GrepOutcome, type GrepRequest, type SearchRuntimeShape,
} from "../src/runtime.ts";
...
async function fixture(run: (cwd: string, search: SearchRuntimeShape) => Promise<void>) {
  ...
  try { await run(cwd, service); }
  finally { await runtime.dispose(); rmSync(cwd, { recursive: true, force: true }); }
}
...
async function grep(cwd: string, request: Omit<GrepRequest, "cwd">) {
  const runtime = createSearchRuntime();
  try { return await runSearch(runtime, runtime.runSync(SearchRuntime).grep({ cwd, ...request })); }
  finally { await runtime.dispose(); }
}

test("auto context keeps ±5 lines for 1–3 matches and disappears for four", { skip: !hasRg }, async () => {
  await fixture(async (cwd) => {
    for (const count of [1, 2, 3, 4]) {
      const hitLines = Array.from({ length: count }, (_, i) => i * 20 + 11);
      writeFileSync(join(cwd, "a.txt"), Array.from({ length: 80 }, (_, i) => hitLines.includes(i + 1) ? "needle" : `line ${i + 1}`).join("\n"));
      ...
      if (count <= 3) assert.deepEqual(outcome.context.slice(0, 10).map((row) => row.lineNumber), [6, 7, 8, 9, 10, 12, 13, 14, 15, 16]);
    }
  });
});

test("explicit context merges overlapping windows and zero disables enrichment", { skip: !hasRg }, async () => {
  await fixture(async (cwd) => {
    writeFileSync(join(cwd, "a.txt"), "start\nbefore\nneedle\nmiddle\nneedle\nafter\nend\n");
    const explicit = await grep(cwd, { pattern: "needle", context: 2 });
    assert.deepEqual(explicit.matches.map((row) => row.lineNumber), [3, 5]);
    assert.deepEqual(explicit.context.map((row) => row.lineNumber), [1, 2, 4, 6, 7]);
    assert.deepEqual((await grep(cwd, { pattern: "needle", context: 0 })).context, []);
    for (const context of [-1, 1.5, 51, NaN]) {
      await assert.rejects(() => grep(cwd, { pattern: "needle", context }), /integer between 0 and 50/);
    }
  });
});
...
test("auto context never enriches incomplete searches and excludes orphan windows", () => {
  const base: GrepOutcome = {
    ...
    truncated: false, timedOut: false, skippedRecords: 0,
  };
  assert.deepEqual(finalizeGrep(base, undefined).context, [base.context[0]]);
  for (const flags of [{ truncated: true }, { timedOut: true }, { skippedRecords: 1 }, { matches: [] }, { output: "files" as const }]) {
    assert.deepEqual(finalizeGrep({ ...base, ...flags }, undefined).context, []);
  }
  assert.deepEqual(finalizeGrep({ ...base, truncated: true }, 2).context, [base.context[0]]);
});

test("files mode uses NUL-delimited rg -l, ignoring context and bypassing the line limit", { skip: !hasRg }, async () => {
  await fixture(async (cwd) => {
    writeFileSync(join(cwd, "a.txt"), "needle\n".repeat(500));
    writeFileSync(join(cwd, "b.txt"), "needle\n");
    const args = buildRgArgs({ cwd, pattern: "needle", output: "files", literal: true, context: 50 }, cwd);
    ...
    const outcome = await grep(cwd, { pattern: "needle", output: "files", context: 50 });
...
test("files mode preserves unusual paths and never overrides ignores with a glob", { skip: !hasRg }, async () => {
  await fixture(async (cwd) => {
...
test("exact limits are complete; one extra record marks the appropriate mode partial", { skip: !hasRg }, async () => {
  await fixture(async (cwd) => {
    ...
    for (let i = 0; i <= GREP_FILE_LIMIT; i++) {
      if (i === GREP_FILE_LIMIT) {
        assert.equal((await grep(cwd, { pattern: "needle", path: "files", output: "files" })).truncated, false);
      }
...
test("literal search treats metacharacters, spaces, and option-like patterns as data", { skip: !hasRg }, async () => {
  await fixture(async (cwd) => {
...
test("long-line previews keep late matches visible with Unicode byte offsets", { skip: !hasRg }, async () => {
  await fixture(async (cwd) => {
    for (const prefix of ["x".repeat(600), "中文😀".repeat(200)]) {
      ...
      assert.ok(result.matches[0]!.text.isWellFormed());
...
test("decoder converts byte offsets after invalid UTF-8 and stripped CRs", () => {
  ...
  const event = decodeRgEvent(JSON.stringify({ type: "match", data: {
    path: { text: "a" }, line_number: 1, lines: { bytes: bytes.toString("base64") },
  ...
  assert.ok(clipLine(text, 802, 808).isWellFormed());
  assert.match(clipLine(text, 802, 808), /TARGET/);
});

test("slash globs have exactly one root, including negation and explicit grep files", { skip: !hasRg || !hasFd }, async () => {
  await fixture(async (cwd, service) => {
    ...
    try {
      for (const pattern of ["src/*.ts", "!src/*.ts"]) {
        ...
        assert.deepEqual((await runSearch(runtime, service.find({ cwd, pattern, path: "src" }))).files, expected);
        for (const output of ["content", "files"] as const) {
          assert.deepEqual((await grep(cwd, { pattern: "needle", glob: pattern, path: "src", output })).files, expected);
        }
      }
      assert.equal((await grep(cwd, { pattern: "needle", path: "src/a.ts", glob: "src/*.ts" })).matches.length, 0);
      assert.equal((await grep(cwd, { pattern: "needle", path: "src/a.ts", glob: "*.ts" })).matches.length, 1);
    } finally { await runtime.dispose(); }
  });
});
