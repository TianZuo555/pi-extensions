/**
 * Model-facing prompt strings and parameter descriptions for the ripgrep/fd
 * search tools.
 *
 ...
 * grep/find prior — the path DSL, exclude syntax, fuzzy-vs-glob find, and the
 * "stop searching, start reading" budget. Restating what the schema already
 * says only buys tokens.
 */
...
export const GREP_TOOL_DESCRIPTION =
  `Search file contents with ripgrep. Smart-case (all-lowercase patterns match case-insensitively), auto-detects regex vs literal, respects .gitignore, and searches hidden files. Default limit ${DEFAULT_GREP_LIMIT} matches.`;

export const GREP_PROMPT_SNIPPET =
  "Search file contents with ripgrep (path/exclude filters, smart-case, context)";
...
export const GREP_PARAMETER_DESCRIPTIONS = {
  pattern:
    "Text to search for. Literal by default; treated as a regex when it contains regex syntax. A pattern that ripgrep's parser rejects is retried literally, so unbalanced or JavaScript-only syntax still finds that text.",
  ...
  context:
    `Lines of context to show before and after each match (default 0, maximum ${MAX_CONTEXT_LINES}).`,
  limit:
...
export const FIND_TOOL_DESCRIPTION =
  `Find files by fuzzy path match or glob, backed by fd. Fuzzy matching scores against the whole repo-relative path, so multiple words narrow the result without having to be adjacent or in order. Respects .gitignore. Default limit ${DEFAULT_FIND_LIMIT}.`;

export const FIND_PROMPT_SNIPPET =
  "Find files by fuzzy path match or glob (fd-backed)";
...
export const FIND_PROMPT_GUIDELINES = [
  ...
  "find: for an exact filename or a known layout, pass a glob in path ('**/profile.h', 'src/**/*.test.ts') — that is a precise filter, while pattern is a fuzzy score.",
...
export const FIND_PARAMETER_DESCRIPTIONS = {
  pattern:
    "Fuzzy path query, scored against the whole repo-relative path. Multiple words all have to match but need not be adjacent or ordered. Pass an empty string to list files matching path alone.",
  path: PATH_DSL_DESCRIPTION,
  exclude: EXCLUDE_DSL_DESCRIPTION,
  limit:
...
export const MULTI_GREP_TOOL_DESCRIPTION =
  "Search file contents for ANY of several literal patterns in one pass (ripgrep matches them together, which is faster than a regex alternation or repeated greps). Patterns are always literal.";
...
export const MULTI_GREP_PARAMETER_DESCRIPTIONS = {
  patterns:
    "Literal strings to search for. A line matching any of them is returned. Include snake_case, camelCase, and PascalCase variants.",
  path: PATH_DSL_DESCRIPTION,
  exclude: EXCLUDE_DSL_DESCRIPTION,
  caseSensitive:
...
export function resultLimitNotice(
  ...
  const continuation = nextLimit > limit
...
export function candidateCapNotice(cap: number): string {
  return `[File walk stopped at ${cap} candidates. Narrow the search with path/exclude for complete fuzzy ranking.]`;
}
