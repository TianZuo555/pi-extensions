...
test("detached descendant groups are killed; root and in-group children survive", {
  skip: !posix,
}, async () => {
...
test("cleanup without detached descendants signals nothing and keeps the tree", {
  skip: !posix,
}, async () => {
  const script = `
const { spawn } = require("node:child_process");
...
test("cleanup for an unknown root pid is a no-op", { skip: !posix }, async () => {
